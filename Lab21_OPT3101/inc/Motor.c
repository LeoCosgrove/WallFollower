...
void Motor_Init(void){
    ...
    PWM_InitRL(initPeriod, initDutyRight, initDutyLeft);   //initialize PWM with 10ms period and 0% duty cycle
    P3->OUT |= 0xC0;
...
void Motor_Stop(void){
  ...
  PWM_DutyRight(0);
  PWM_DutyLeft(0);
 // P3->OUT &= ~0xC0;                     //disable full power mode
...
void Motor_Forward(uint16_t leftDuty, uint16_t rightDuty){ 
   ...
   // P3->OUT |= 0xC0;                     //enable full power mode
    PWM_DutyRight(rightDuty);
    PWM_DutyLeft(leftDuty);
...
void Motor_Right(uint16_t leftDuty, uint16_t rightDuty){ 
    P5->OUT |= 0x20;                    // Set right wheel direction to backwards - 1 on (P5.5)
    P5->OUT &= ~0x10;                   // Set left wheel direction to forwards - 0 on (P5.4)
   // P3->OUT |= 0xC0;                     //enable full power mode
    PWM_DutyRight(rightDuty);
    PWM_DutyLeft(leftDuty);
}
...
void Motor_Left(uint16_t leftDuty, uint16_t rightDuty){ 
    P5->OUT &= ~0x20;                    // Set right wheel direction to forwards - 0 on (P5.5)
    P5->OUT |= 0x10;                     // Set left wheel direction to backwards - 1 on (P5.4)
  //  P3->OUT |= 0xC0;                     //enable full power mode
    PWM_DutyRight(rightDuty);
    PWM_DutyLeft(leftDuty);
}
...
void Motor_Backward(uint16_t leftDuty, uint16_t rightDuty){ 
    ...
    P5->OUT |= 0x30;             //set output to 1 (backwards)
  //  P3->OUT |= 0xC0;                     //enable full power mode
    PWM_DutyRight(rightDuty);
    PWM_DutyLeft(leftDuty);
